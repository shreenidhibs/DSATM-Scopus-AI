...
def record(row: pd.Series, mapping: dict[str, str | None]) -> dict[str, Any]:
    ...
    return {
        ...
        "citations": citations_num, "doi": doi,
        "doi_url": f"https://doi.org/{doi}" if doi else "",
        "document_type": val("document_type"),
        "eid": val("eid"),
        "scopus_url": (
            f"https://www.scopus.com/record/display.uri?eid={val('eid')}&origin=resultslist"
            if val("eid") else ""
        ),
        "link": link, "abstract": val("abstract"),
